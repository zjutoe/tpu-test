#!/usr/bin/env python3
"""
Simple MNIST Training with XLA CPU fallback
Modified version that works around TPU initialization issues
"""

import os
# Force CPU backend to avoid TPU initialization issues
...
def train_epoch(model, device, train_loader, optimizer, criterion, epoch):
    """Train for one epoch using XLA-optimized patterns"""
    model.train()
...
def main():
    ...
    print("🚀 Starting MNIST Training with XLA (CPU backend)")
    ...
    # Get XLA device (should be CPU-based)
    devices = xm.get_xla_supported_devices()
    device = devices[0] if devices else "cpu"
    print(f"Using device: {device}")
    print(f"Available XLA devices: {devices}")
    
    # Create model and move to XLA device
    model = SimpleCNN(num_classes=10).to(device)
    print("✅ Model created and moved to XLA device")
    ...
    # Save model
    model_path = 'mnist_xla_model.pth'
